...
import { ledger, type Ledger, State } from '../../contract/src/managed/anon-feed/contract/index.js';
...
import { AnonFeedPrivateState } from '../../contract/src/witnesses.js';

// @ts-expect-error: It's needed to enable WebSocket usage through apollo
globalThis.WebSocket = WebSocket;

/* **********************************************************************
 * getAnonFeedLedgerState: a helper that queries the current state of
...
export const getAnonFeedLedgerState = async (
...
const deployOrJoin = async (
  ...
  while (true) {
    ...
    switch (choice) {
      case '1':
        api = await AnonFeedAPI.deploy(providers, logger);
        logger.info(`Deployed contract at address: ${api.deployedContractAddress}`);
        return api;
      case '2':
        api = await AnonFeedAPI.join(providers, await rli.question('What is the contract address (in hex)? '), logger);
...
const displayLedgerState = async (
  providers: AnonFeedProviders,
  DeployedAnonFeedContract: DeployedAnonFeedContract,
  logger: Logger,
): Promise<void> => {
  const contractAddress = DeployedAnonFeedContract.deployTxData.public.contractAddress;
  const ledgerState = await getAnonFeedLedgerState(providers, contractAddress);
  if (ledgerState === null) {
...
const displayPrivateState = async (providers: AnonFeedProviders, logger: Logger): Promise<void> => {
  const privateState = await providers.privateStateProvider.get(AnonFeedPrivateStateKey);
...
const displayDerivedState = (ledgerState: AnonFeedDerivedState | undefined, logger: Logger) => {
...
const mainLoop = async (providers: AnonFeedProviders, rli: Interface, logger: Logger): Promise<void> => {
  const AnonFeedAPI = await deployOrJoin(providers, rli, logger);
  if (AnonFeedAPI === null) {
    return;
  }
  let currentState: AnonFeedDerivedState | undefined;
  const stateObserver = {
    next: (state: AnonFeedDerivedState) => (currentState = state),
  };
  const subscription = AnonFeedAPI.state$.subscribe(stateObserver);
  try {
    while (true) {
      ...
      try {
        switch (choice) {
          case '1': {
            ...
            await AnonFeedAPI.post(message);
            break;
          }
          case '2':
            await AnonFeedAPI.takeDown();
            break;
          case '3':
            await displayLedgerState(providers, AnonFeedAPI.deployedContract, logger);
            break;
          case '4':
